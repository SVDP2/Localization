...
public:
      ...
      rclcpp::NodeOptions(options).automatically_declare_parameters_from_overrides(true))
  {
    ...
    this->declare_parameter<bool>("use_hpposllh_accuracy_fallback", true);
    this->declare_parameter<double>("nav_cov_timeout_sec", 0.5);
    use_hpposllh_accuracy_fallback_ =
      this->get_parameter("use_hpposllh_accuracy_fallback").as_bool();
    nav_cov_timeout_sec_ = std::max(0.0, this->get_parameter("nav_cov_timeout_sec").as_double());
...
private:
  ...
  rclcpp::Time last_nav_cov_time_;
    ...
    const ublox_ubx_msgs::msg::UBXNavHPPosLLH::SharedPtr ubx_hppos_llh_msg)
  {
    ...
    // Fill in covariance data. Prefer full NAV-COV when it is fresh; otherwise
    // fall back to per-fix HPPOSLLH accuracy so covariance updates with position.
    ...
    const bool use_nav_cov = nav_cov_is_fresh();
    if (use_nav_cov) {
    ...
    } else if (use_hpposllh_accuracy_fallback_) {
      fill_accuracy_fallback_covariance(nav_sat_fix_msg, ubx_hppos_llh_msg);
    } else {
  ...
  void nav_cov_callback(const ublox_ubx_msgs::msg::UBXNavCov::SharedPtr ubx_cov_msg)
  {
    ...
    last_nav_cov_time_ = this->get_clock()->now();
  }

  UBLOX_NAV_SAT_FIX_HP_NODE_LOCAL
  bool nav_cov_is_fresh()
  {
    if (!have_recd_enu_pos_cov_) {
      return false;
    }
    if (nav_cov_timeout_sec_ <= 0.0) {
      return true;
    }
    const double age_sec =
      (this->get_clock()->now() - last_nav_cov_time_).nanoseconds() * 1.0e-9;
    return std::isfinite(age_sec) && age_sec <= nav_cov_timeout_sec_;
  }

  UBLOX_NAV_SAT_FIX_HP_NODE_LOCAL
  void fill_accuracy_fallback_covariance(
    sensor_msgs::msg::NavSatFix & nav_sat_fix_msg,
    const ublox_ubx_msgs::msg::UBXNavHPPosLLH::SharedPtr ubx_hppos_llh_msg)
  {
    for (size_t i = 0; i < nav_sat_fix_msg.position_covariance.size(); i++) {
      nav_sat_fix_msg.position_covariance[i] = 0.0;
    }

    const double h_acc_m = static_cast<double>(ubx_hppos_llh_msg->h_acc) * 1.0e-4;
    const double v_acc_m = static_cast<double>(ubx_hppos_llh_msg->v_acc) * 1.0e-4;
    ...
    const double h_var = h_acc_m * h_acc_m;
    const double v_var = v_acc_m * v_acc_m;
    nav_sat_fix_msg.position_covariance[0] = h_var;
    nav_sat_fix_msg.position_covariance[4] = h_var;
    nav_sat_fix_msg.position_covariance[8] = v_var;
    nav_sat_fix_msg.position_covariance_type =
