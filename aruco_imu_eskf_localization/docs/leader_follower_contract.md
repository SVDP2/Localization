...
- ESKF fused state와 controller/debug 출력은 절대좌표가 아니라 `leader/leader_rear -> follower/base_link` 상대좌표이다.
- `map`은 leader/follower GPS 차이를 만들기 위한 중간 계산 frame이다. Follower controller가 `map` pose를 직접 소비하면 안 된다.
...
- follower GPS absolute intermediate: `/follower/localization/gps/odom`
- leader GPS/odom absolute intermediate: `/leader/localization/gps/odom`
